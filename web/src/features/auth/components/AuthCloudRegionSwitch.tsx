        ...
        <div>
          ...
          {isSignUpPage && cloudRegion === "HIPAA" ? (
            <p className="text-muted-foreground text-xs">
              {t("Demo project is not available in the HIPAA data region.")}
            </p>
          ) : null}
        </div>
        <Select
        ...
        {cloudRegion === "HIPAA" && (
          <div className="bg-muted/50 text-muted-foreground mt-2 rounded-md p-3 text-xs">
            <p>
              ...
              >
                {t("Learn more about HIPAA compliance →")}
              </a>
            </p>
          </div>
        )}
...
const DataRegionInfo = () => {
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("Data Regions")}</DialogTitle>
        </DialogHeader>
        <DialogBody>
          <DialogDescription className="flex flex-col gap-2">
            <p>{t("Litefuse Cloud is available in three data regions:")}</p>
            <ul className="list-disc pl-5">
              <li>{t("US: Oregon (AWS us-west-2)")}</li>
              <li>{t("EU: Ireland (AWS eu-west-1)")}</li>
              <li>
                {t(
