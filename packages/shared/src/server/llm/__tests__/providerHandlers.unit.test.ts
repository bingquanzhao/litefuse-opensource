...
describe("OpenAI-compatible provider handlers", () => {
  ...
  it("builds a compatible deepseek-v4-pro evaluation configuration", () => {
    const handler = resolveOpenAICompatibleProviderHandler({
      adapter: LLMAdapter.OpenAI,
      provider: "deepseek",
    ...
    expect(handler?.id).toBe("deepseek-openai");
  ...
  it("uses JSON mode for DeepSeek evaluations without sending tool_choice", async () => {
    const handler = resolveOpenAICompatibleProviderHandler({
      adapter: LLMAdapter.OpenAI,
      provider: "deepseek",
      baseURL: "https://api.deepseek.com/v1",
  ...
  it("matches DeepSeek's official hostname when the provider is custom", () => {
    const handler = resolveOpenAICompatibleProviderHandler({
      adapter: LLMAdapter.OpenAI,
      provider: "custom",
      baseURL: "https://api.deepseek.com/v1",
  ...
  it("prioritizes DeepSeek's official hostname over a mismatched provider", () => {
    const handler = resolveOpenAICompatibleProviderHandler({
      adapter: LLMAdapter.OpenAI,
      provider: "minimax",
      baseURL: "https://api.deepseek.com/v1",
  ...
  it("preserves DeepSeek thinking settings for non-structured requests", () => {
    const handler = resolveOpenAICompatibleProviderHandler({
      adapter: LLMAdapter.OpenAI,
      provider: "deepseek",
      baseURL: "https://api.deepseek.com/v1",
    ...
    expect(config?.structuredOutput).toBeUndefined();
  });
